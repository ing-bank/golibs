...
func newServer() *gin.Engine {
	...
	router := gin.New()
	router.Use(Gzip(int(DefaultCompression)))
...
func TestVaryHeader(t *testing.T) {
	req, _ := http.NewRequestWithContext(t.Context(), "GET", "/ping", nil)
...
func TestGzipPNG(t *testing.T) {
	req, _ := http.NewRequestWithContext(t.Context(), "GET", "/image.png", nil)
	req.Header.Add(headerAcceptEncoding, "gzip")
...
func TestExcludedPathsAndExtensions(t *testing.T) {
	...
	for _, tt := range tests {
		req, _ := http.NewRequestWithContext(t.Context(), "GET", tt.path, nil)
		req.Header.Add(headerAcceptEncoding, "gzip")

		router := gin.New()
		router.Use(Gzip(int(DefaultCompression), tt.option))
...
func TestGzipWithReverseProxy(t *testing.T) {
	req, _ := http.NewRequestWithContext(t.Context(), "GET", "/reverse", nil)
	req.Header.Add(headerAcceptEncoding, "gzip")
...
func TestDecompressGzip(t *testing.T) {
	buf := &bytes.Buffer{}
	gz, _ := gzip.NewWriterLevel(buf, int(DefaultCompression))
	...
	req, _ := http.NewRequestWithContext(t.Context(), "POST", "/", buf)
	req.Header.Add(headerContentEncoding, "gzip")

	router := gin.New()
	router.Use(Gzip(int(DefaultCompression), WithDecompressFn(DefaultDecompressHandle)))
...
func TestDecompressGzipWithEmptyBody(t *testing.T) {
	req, _ := http.NewRequestWithContext(t.Context(), "POST", "/", nil)
	req.Header.Add(headerContentEncoding, "gzip")
...
func TestDecompressGzipWithIncorrectData(t *testing.T) {
	req, _ := http.NewRequestWithContext(t.Context(), "POST", "/", bytes.NewReader([]byte(testResponse)))
	req.Header.Add(headerContentEncoding, "gzip")
...
func TestDecompressOnly(t *testing.T) {
	...
	router := gin.New()
	router.Use(Gzip(int(NoCompression), WithDecompressOnly(), WithDecompressFn(DefaultDecompressHandle)))
	router.POST("/", func(c *gin.Context) {
		if v := c.Request.Header.Get(headerContentEncoding); v != "" {
			t.Errorf("unexpected `Content-Encoding`: %s header", v)
		}
...
func TestGzipWithDecompressOnly(t *testing.T) {
	...
	r := gin.New()
	r.Use(Gzip(int(NoCompression), WithDecompressOnly(), WithDecompressFn(DefaultDecompressHandle)))
